...
def welcome(message):
	...
	if ref:
		with sqlite3.connect('ref.db') as con:
			cur = con.cursor()
			cur.execute("UPDATE users SET ref_to = ref_to + 1, ref_proc = ref_proc + 10 WHERE id_user=?", (ref,))
			con.commit()
			name = message.from_user.first_name
			bot.send_message(ref, str(name) + ", перешел по вашей реферальной ссылке.\nВы получите бонус 10% за следующую покупку.")
	else:
		pass
	people_id = message.chat.id
	with open('All/id.txt') as f:
		found = False
		for line in f:
			if str(people_id) in str(line): # Key line: check if `w` is in the line.
				found = True
		if not found:
...
def adminka(message):
	if message.from_user.id == config.admin_id:
		...
		keyboard.add(types.InlineKeyboardButton(text="Рассылка 💎", callback_data ='Rassilka'))
		keyboard.add(types.InlineKeyboardButton(text="Посчитать кол-во пользователей 🗽", callback_data ='AllUsers'))
		adminmessage = bot.send_message(message.chat.id, "Админ меню успешно открыто! 🗡", reply_markup=keyboard)
	else:
...
def main(message):
	if message.chat.type == 'private':
		...
		if message.text == 'GTA5RP | 200р':
			...
			bot.send_message(message.chat.id, "Выбери сервер проекта *GTA5RP*, на котором Вы играете.", reply_markup=markup, parse_mode="Markdown")
		if message.text == 'Downtown':
			...
			markup.row(menu1, menu2, menu3)
			bot.send_message(message.chat.id, "Введите количество виртов,которое вы хотите купить или воспользуйтесь клавиатурой.", reply_markup=markup)
		...
		if message.text == 'Majestic | 70р':
			...
			bot.send_message(message.chat.id, "Выбери сервер проекта *Majestic*, на котором Вы играете.", reply_markup=markup, parse_mode="Markdown")
		...
		if message.text == 'RedAge | 40р':
			...
			bot.send_message(message.chat.id, "Выбери сервер проекта *RedAge*, на котором Вы играете.", reply_markup=markup, parse_mode="Markdown")
		...
		if message.text == 'Arizona RP | 30р':
			...
			bot.send_message(message.chat.id, "Выбери сервер проекта *Arizona RP*, на котором Вы играете.", reply_markup=markup, parse_mode="Markdown")
		...
		if message.text == 'Radmir RP | 10р':
			...
			bot.send_message(message.chat.id, "Выбери сервер проекта *Radmir RP*, на котором Вы играете.", reply_markup=markup, parse_mode="Markdown")
		...
		if message.text == '100000':
			...
			msg = bot.send_message(message.chat.id, "Введите свой игровой ник в формате Имя_Фамилия.", reply_markup=types.ReplyKeyboardRemove())
...
def decyatka1(message):
	...
	msg = bot.send_message(message.chat.id, 'Введите свои контакты для связи с вами (Discord, Telegram, Vk и тд)')
...
def decyatka2(message):
	...
	url_button = types.InlineKeyboardButton(text="Быстрая оплата", url="https://qiwi.com/payment/form/99?extra%5B%27account%27%5D=" + str(config.number_qiwi) + "&amountInteger=" + str(cena) +"&amountFraction=0&currency=643&extra%5B%27comment%27%5D=" + str(Name_Surname) + " [ " + str(zaen) + " ]%20|%20" + str(zaen2) + "&blocked[0]=sum&blocked[1]=account&blocked[2]=comment")
	ProverkaPlatejki = types.InlineKeyboardButton(text="Проверить платеж", callback_data ='Platejka')
	...
	time.sleep(900)
	bot.send_message(message.chat.id, message.from_user.first_name + " , Не обнаружили поступления на счет. Если это ошибка, обратитесь по контактам.")
def RassAll(message):
	keyboard=types.InlineKeyboardMarkup()
	keyboard.add(types.InlineKeyboardButton(text="Рассылка 💎", callback_data ='Rassilka'))
	keyboard.add(types.InlineKeyboardButton(text="Посчитать кол-во пользователей 🗽", callback_data ='AllUsers'))
	...
	bot.send_message(config.admin_id, "Рассылка была успешно закончена! 💎", reply_markup=keyboard)
@bot.callback_query_handler(func=lambda call: True)
def platejka(call):
	if call.data == 'Platejka':
		bot.answer_callback_query(callback_query_id=call.id, show_alert=True, text="Средства еще не дошли до нас!\nЕсли в течении часа средства не поступят на ваш счет, обратитесь по контактам 🙈")
	...
	if call.data == 'AllUsers':
		...
		keyboard.add(types.InlineKeyboardButton(text="Посчитать кол-во пользователей 🗽", callback_data ='AllUsers'))
		bot.delete_message(call.message.chat.id, call.message.message_id)
		fhand=open('All/id.txt') # Opens the file and puts the content of it in the "fhand" variable
		count=0 # Creates new variable "count" and sets it to 0
		for line in fhand:
			count=count+1 # Increase count by 1 for every line in the file

		bot.send_message(call.message.chat.id, "Кол-во пользователей в боте: " + str(count), reply_markup=keyboard)
